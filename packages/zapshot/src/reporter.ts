...
export const report = (
  ...
  for (let caseName in report.cases) {
    ...
    if (quiet) {
      console.log(
        `${caseName}: ${c.total.toFixed(precision)}ms (mean: ${c.mean.toFixed(
          precision
        )}ms) ${humanizeDiff(c.diff, c.diffPercentage, "ms", {
          precision,
          threshold
        })}`
      );
      ...
      for (let mark of c.marks) {
        ...
        console.log(
          `${percentage.padStart(legend.length - 1)}% ${mark.name.padStart(
            longestMarkWidth
          )}: ${mark.total.toFixed(precision).padStart(totalMaxWidth)}ms (${
            mark.times
          } times) ${humanizeDiff(mark.diff, mark.diffPercentage, "ms", {
            precision,
            threshold
          })}`
        );
