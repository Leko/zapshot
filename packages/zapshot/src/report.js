...
export class Report {
  ...
  static fromMetricses(metrics: Metrics, beforeMetrics: ?Metrics): Report {
    const cases: { [caseName: string]: Case } = metrics
      .getItems()
      .reduce((acc, metric) => {
        const before: ?Metric = beforeMetrics
        ...
        const marks = metric.getMarks().reduce((marks, mark) => {
          ...
          const subTotalDiff = diff(metric, before, m =>
            m.getMarkedTotal(mark)
          );
          ...
          return marks.concat([
            {
              ...
              diff: subTotalDiff,
              diffPercentage: subTotalDiff
                ? subTotalDiff / subTotal * 100
                : null
            }
          ]);
        }, []);
